...
export function conter_cruzadinhas(){
	let allsectionEl = document.querySelectorAll(".sections");
	var menor_top = Number.MAX_VALUE;
	var maior_right = Number.MIN_VALUE;
	var maior_bottom = Number.MIN_VALUE;
	var menor_left = Number.MAX_VALUE;

	for(var i = 0; i < allsectionEl.length; i++){
		var width = parseFloat(allsectionEl[i].offsetWidth) /*/ window.innerWidth) * 100;*/;
		var height = parseFloat(allsectionEl[i].offsetHeight) /*/ window.innerHeight) * 100*/;
		var top = parseFloat(allsectionEl[i].style.top);
		var left = parseFloat(allsectionEl[i].style.left);
		...
		if(maior_right < right){
			maior_right = right;
		}
		if(maior_bottom < bottom){
			maior_bottom = bottom;
		}
		if(menor_left > left){
			menor_left = left;
		}
		//container_cruzadinhasEl.appendChild(allsectionEl[i]);
	}
	conterCruzadinhasEl.y = menor_top + 'px';
	conterCruzadinhasEl.x = menor_left + 'px';
	//container_cruzadinhasEl.style.top = menor_top + 'px';
	//container_cruzadinhasEl.style.left = menor_left + 'px';
	container_cruzadinhasEl.style.width = (maior_right - menor_left) + 'px';
	container_cruzadinhasEl.style.height = (maior_bottom - menor_top) + 'px';

    area_restanteEl.style.alignItems = "center";
    area_restanteEl.style.justifyContent = "center";
    container_cruzadinhasEl.style.transformOrigin = "unset";
	//container_cruzadinhasEl.style.top = 0+ 'px';
	//container_cruzadinhasEl.style.left = 0+ 'px';
	
	for(var i = 0; i < allsectionEl.length; i++){
		allsectionEl[i].style.left = parseFloat(allsectionEl[i].style.left) - menor_left + 'px';
		allsectionEl[i].style.top = parseFloat(allsectionEl[i].style.top) - menor_top + 'px';  
	}

	container_cruzadinhasEl.style.cssText += 'margin-top: 3px;';
